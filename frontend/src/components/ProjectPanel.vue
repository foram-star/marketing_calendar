        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <input type="date" v-model="form.due_date" class="rounded-md border border-gray-200 px-2.5 py-1.5 text-[12.5px] outline-none" />
            </div>
          </div>
          ...
          <div v-if="project" class="flex flex-col gap-1.5 border-t border-gray-100 pt-4">
            <label class="text-[12px] font-medium text-gray-700">
              Tasks<template v-if="projectTasks.length"> <span class="text-gray-400">· {{ projectTasks.filter((t) => t.done).length }}/{{ projectTasks.length }} done</span></template>
            </label>
            <div class="flex items-center gap-2">
              <input
                v-model="newTaskTitle"
                placeholder="Add a task…"
                class="flex-1 rounded-md border border-gray-200 px-2.5 py-1.5 text-[12.5px] outline-none"
            ...
            <div class="flex flex-col gap-1">
              ...
              <p v-if="!projectTasks.length" class="m-0 py-1 text-[11.5px] text-gray-400">No tasks linked yet.</p>
            </div>
          </div>
        </div>
      </div>

...
<template>
  <header class="flex items-center gap-3.5 border-b border-gray-100 bg-white px-6 pb-3.5 pt-4">
    <div class="flex flex-col gap-0.5">
      ...
      <span class="text-[12px] text-gray-400">Operational overview — publish outcomes we've recorded ourselves</span>
    </div>
  </header>
  ...
  <div class="flex-1 overflow-auto p-6">
    <div class="mb-5 grid grid-cols-4 gap-4">
      <div v-for="k in kpis" :key="k.label" class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
        <span class="text-[11.5px] font-medium text-gray-400">{{ k.label }}</span>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="rounded-xl border border-gray-100 bg-white p-2 shadow-sm">
        ...
        />
        <div v-else class="flex h-[220px] items-center justify-center text-[12.5px] text-gray-400">No posts yet</div>
      </div>
      <div class="rounded-xl border border-gray-100 bg-white p-2 shadow-sm">
        <DonutChart
        ...
        />
        <div v-else class="flex h-[220px] items-center justify-center text-[12.5px] text-gray-400">No platforms selected yet</div>
      </div>
    </div>
    ...
    <div class="rounded-xl border border-gray-100 bg-white shadow-sm">
      <div class="border-b border-gray-100 px-4 py-3">
        ...
        <p class="m-0 mt-0.5 text-[11.5px] text-gray-400">Likes, reach, comments and saves pulled from each connected platform</p>
      </div>
      <div class="flex flex-col items-center gap-2 px-4 py-10 text-center">
        <span class="rounded-full bg-gray-100 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-gray-400">Coming soon</span>
        <p class="m-0 max-w-[380px] text-[12.5px] text-gray-400">
    ...
    <div class="rounded-xl border border-gray-100 bg-white shadow-sm">
      ...
      <div v-if="(analyticsResource.data?.recent_failures || []).length">
        ...
        >
          ...
          <span class="max-w-[320px] truncate text-[11.5px] text-gray-400">{{ f.error_message }}</span>
          <span class="text-[10.5px] text-gray-400">{{ fmtFailedAt(f.last_attempted_at) }}</span>
        </div>
      </div>
      <div v-else class="px-4 py-8 text-center text-[12.5px] text-gray-400">Nothing's failed — clean record.</div>
    </div>
  </div>
</template>
